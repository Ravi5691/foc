...
const ImageSwitcher = () => {
  ...
  return (
    <div className='flex flex-col justify-center items-center gap-10 min-h-screen p-5 md:p-10'>
      <div className='w-full max-w-[800px] flex justify-center'>
        <h1 className='text-white text-3xl sm:text-4xl md:text-5xl font-bold leading-relaxed text-center'>
          How it <span className='text-green'>Works</span>
        </h1>
      </div>
      ...
      <div className="w-full max-w-[1300px] flex flex-col md:flex-row gap-10 p-5 flex-1">
        ...
        <div className="flex flex-col flex-1 gap-5">
          {items.map((item, index) => (
            <div className='flex justify-center flex-1' key={index}>
              <span
                className="cursor-pointer bg-[#3C5557] hover:bg-green border-4 border-[#1D2B2C] p-4 rounded-xl w-full h-[150px] sm:h-[200px] flex items-center justify-center text-center md:p-6 lg:p-8"
                onMouseEnter={() => changeImage(item.image)}
              >
                {item.text} {/* Dynamic text */}
              </span>
        ...
        <div className="flex-1 flex justify-center items-center relative overflow-hidden">
          <img
            src={imageSrc}
            alt="Switchable"
            className={`rounded-xl h-[200px] sm:h-[300px] md:h-full transition-transform duration-1000 ease-in-out object-cover`} // Smooth transition for image
          />
        </div>
      </div>
    </div>
